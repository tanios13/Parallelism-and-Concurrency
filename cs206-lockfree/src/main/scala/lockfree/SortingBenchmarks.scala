...
object SortingBenchmarks extends App:
  ...
  val syncSpeedup: Float = syncSeqTime.toFloat / syncParTime
  val fineSpeedup: Float = fineSeqTime.toFloat / fineParTime
  val syncOverFineSpeedup: Float = syncParTime.toFloat / fineParTime
  ...
  println(
    f"""
      ...
      |  - speedup:     $syncSpeedup%.2f
      |
      |fine-grained:
      |  - sequential:  $fineSeqTime
      |  - parallel:    $fineParTime
      |  - speedup:     $fineSpeedup%.2f
      |
      |synchronized/fine-grained:
      |  - speedup:     $syncOverFineSpeedup%.2f
      |
    """.stripMargin)
