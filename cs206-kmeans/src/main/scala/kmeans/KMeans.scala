...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    val pToMeans = points.groupBy(p => findClosest(p, means))
    means.map(m => m -> pToMeans.getOrElse(m, ParSeq())).toMap
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] =
    oldMeans.map(o => findAverage(o, classified(o)))

  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean = 
    if(oldMeans.isEmpty) then true
    else oldMeans.head.squareDistance(newMeans.head) <= eta & converged(eta, oldMeans.tail, newMeans.tail)

  @tailrec
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    if !converged(eta, means, update(classify(points, means), means)) then kMeans(points, update(classify(points, means), means), eta) else update(classify(points, means), means) // your implementation need to be tail recursive
