...
extends Quad:
  ...
  val mass: Float = bodies.foldLeft(0f)((_mass, child) => child.mass + _mass)

  val massX: Float = bodies.foldLeft(0f)((_massX, child) => child.mass * child.x + _massX) / mass

  val massY: Float = bodies.foldLeft(0f)((_massY, child) => child.mass * child.y + _massY) / mass
  ...
  def insert(b: Body): Quad = 
    if(size <= minimumSize) then Leaf(centerX, centerY, size, b +: bodies)
      ...
      (b +: bodies).foldLeft(Fork(nw, ne, sw, se))((fork, body) => fork.insert(body)) 
  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Fork(nw, ne, sw, se) =>
        ...
        val dist = (quad.size / distance(quad.massX, quad.massY, x, y)) < theta
        // see if node is far enough from the body,
        if(dist) then addForce(quad.mass, quad.massX, quad.massY)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    var (x, y): (Int, Int) = (((b.x - boundaries.minX) / sectorSize).floor.toInt, ((b.y - boundaries.minY) / sectorSize).floor.toInt)

    if(x > sectorPrecision) then x = sectorPrecision - 1
    if(x < 0) then x = 0
    if(y > sectorPrecision) then y = sectorPrecision - 1
    if(y < 0) then y = 0
  ...
  def combine(that: SectorMatrix): SectorMatrix =
    for i <- 0 until matrix.length do matrix(i) = matrix(i).combine(that.matrix(i))
    this
